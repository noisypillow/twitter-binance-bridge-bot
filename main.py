...
def get_quantity():
    symbol_info = requests.get(URL + f"ticker/price?symbol={SYMBOL}")
...
def get_asset_balance():
    ...
    signature = hmac.new(SECRET_KEY, TOTAL_PARAMS_b, hashlib.sha256).hexdigest()
...
def get_order_status(order_id):
    TOTAL_PARAMS = f"symbol={SYMBOL}&orderId={order_id}&timestamp={get_timestamp()}"
    TOTAL_PARAMS_b = TOTAL_PARAMS.encode('ASCII')
    signature = hmac.new(SECRET_KEY, TOTAL_PARAMS_b, hashlib.sha256).hexdigest()
    payload = TOTAL_PARAMS + "&signature=" + signature
    headers = {'X-MBX-APIKEY': PUB_KEY}
...
def buy(retry):
    ...
    TOTAL_PARAMS = f"symbol={SYMBOL}&side=BUY&type=MARKET&quantity={quantity}&recvWindow=3000&timestamp={get_timestamp()}"
    TOTAL_PARAMS_b = TOTAL_PARAMS.encode('ASCII')
    signature = hmac.new(SECRET_KEY, TOTAL_PARAMS_b, hashlib.sha256).hexdigest()
    payload = TOTAL_PARAMS + "&signature=" + signature
    headers = {'X-MBX-APIKEY': PUB_KEY}
    ...
    if response['status'] == 'FILLED':
        print(datetime.now().strftime("\033[92m%d-%m-%Y %H:%M:%S | \033[0m") + f"\033[92m{str_to_print}\033[0m")
        return response['orderId'], commission
    elif retry > 0:
        print(datetime.now().strftime("\033[91m%d-%m-%Y %H:%M:%S | \033[0m") + "\033[91mOrder error\033[0m")
...
def sell(order_id, retry, commission):
    ...
    TOTAL_PARAMS = f"symbol={SYMBOL}&side=SELL&type=MARKET&quantity={quantity}&recvWindow=4000&timestamp={get_timestamp()}"
    TOTAL_PARAMS_b = TOTAL_PARAMS.encode('ASCII')
    signature = hmac.new(SECRET_KEY, TOTAL_PARAMS_b, hashlib.sha256).hexdigest()
    payload = TOTAL_PARAMS + "&signature=" + signature
    headers = {'X-MBX-APIKEY': PUB_KEY}
    ...
    str_to_print, commission = get_details(response)
    if response['status'] == 'FILLED':
        print(datetime.now().strftime("\033[94m%d-%m-%Y %H:%M:%S | \033[0m") + f"\033[94m{str_to_print}\033[0m")
    elif retry > 0:
        print(datetime.now().strftime("\033[91m%d-%m-%Y %H:%M:%S | \033[0m") + "\033[91mOrder error\033[0m")
        print(r.content.decode())
        print("\033[91mRetrying...\033[0m")
...
def got_tweet(status):
    if get_asset_balance() > ASSET[1]:
        print(datetime.now().strftime(f"%d-%m-%Y %H:%M:%S | ") + "Elon just tweeted about $DOGE:")
        print("    " + status.text)
